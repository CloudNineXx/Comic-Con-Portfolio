...
export function EducationSection() {
  ...
  return (
    <section id="education" className="py-16 bg-gradient-to-br from-yellow-400 via-amber-500 to-orange-600 relative z-30">
      ...
      <div className="container mx-auto px-6 relative z-30">
        ...
        {/* Education Timeline with Stacking Effect */}
        <div className="relative max-w-5xl mx-auto h-auto min-h-[300px] mb-16">
          {education.map((edu, index) => {
            // Calculate stacking positions with reduced spacing
            const stackOffset = stackedCards ? (index * 20) : (index * 100);
            const stackRotation = stackedCards ? (index * 2 - 2) : 0;
            const stackScale = stackedCards ? (1 - index * 0.05) : 1;
            const stackZ = stackedCards ? (education.length - index) : 1;
            const stackOpacity = stackedCards && index > 0 ? 0.8 : 1;
            ...
            return (
              <div
                ...
                style={{
                  ...
                  top: stackedCards ? '0px' : `${index * 100}px`,
                  transitionDelay: `${index * 100}ms`
                }}
              >
                <Card className="border-6 border-black shadow-2xl bg-gradient-to-br from-gray-50 to-white overflow-hidden rounded-2xl hover:scale-102 transition-all duration-300 relative">
                  
                  {/* Status Badge */}
                  <div className="absolute top-4 right-4 z-20">
                    <Badge className={`px-3 py-1 typography-caption border-2 border-black shadow-lg ${
                  ...
                  <CardHeader className={`bg-gradient-to-r ${edu.color} text-white py-3 pb-3 relative overflow-hidden`}>
                    ...
                    <div className="relative z-10 flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <CardTitle className="typography-heading-sm text-white text-comic-shadow mb-2 leading-tight">
                          {edu.institution}
                        </CardTitle>
                        <p className="typography-body text-gray-100 mb-2 leading-snug">
                          {edu.degree}
                        </p>
        ...
        {/* Spacer to ensure proper layout - reduced height */}
        <div className={`transition-all duration-1000 ${stackedCards ? 'h-8' : 'h-40'}`}></div>
      </div>
    </section>
  );
}